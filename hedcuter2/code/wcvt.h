...
private:
	...
	void vor(cv::Mat &  img, cv::Size res, cv::Mat resizedImg, cv::Mat dist,cv::Mat root, cv::Mat visited);
	...
	inline float move_sites(cv::Mat &  img, VorCell & cell, cv::Mat resizedImg)
	{
	...
	inline float move_sites(cv::Mat &  img, cv::Mat resizedImg)
	{
		...
		if (average_termination)
		{
			for (auto& cell : this->cells)
			{
				...
				float offset = move_sites(img, cell, resizedImg);
				max_offset += offset;
			}
