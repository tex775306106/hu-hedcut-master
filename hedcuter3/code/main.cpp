...
int main(int argc, char ** argv)
{
	...
	cv::Mat image = cv::imread(img_filename.c_str() );   // Read the file
	bool png=false;
	string m=img_filename.c_str();
	if(m.find(".png")>0)
		png=true;
	if(png){
		//cv::Mat image = cv::imread(img_filename.c_str() );   // Read the file
		cv::Mat top = cv::imread(img_filename.c_str() );   
		cv::Mat mask = cv::imread(img_filename.c_str() ,0);   
		IplImage* white = cvCreateImage(cvSize(top.cols, top.rows), IPL_DEPTH_8U, 3);
		for (int i = 0; i < white->height; i++)  
		{  
			uchar *ptrImage = (uchar*)(white->imageData + i * white->widthStep);  
		...
		cvSaveImage("white.jpg", white);   
		image = cv::imread("white.jpg"); 
		cv::Mat imageROI;
		imageROI = image(cv::Rect(0,0,top.cols,top.rows));
		top.copyTo(imageROI,mask);
		remove("white.jpg");
	}
