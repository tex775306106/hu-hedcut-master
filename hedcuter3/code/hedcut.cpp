...
void Hedcut::create_disks(cv::Mat & img, CVT & cvt)
{
	...
	for (auto& cell : cvt.getCells())
	{
		...
		for (auto & resizedPix : cell.coverage)
		{
			...
			img.at<cv::Vec4b>(pix.x, pix.y)[3]=255;			
			r += img.at<cv::Vec4b>(pix.x, pix.y)[2];
			g += img.at<cv::Vec4b>(pix.x, pix.y)[1];
			b += img.at<cv::Vec4b>(pix.x, pix.y)[0];
		}
		...
		a = floor(a / cell.coverage.size());
		//a=255;
		//create a disk
		HedcutDisk disk;
		disk.center.x = cell.site.y; //x = col
		disk.center.y = cell.site.x; //y = row
		disk.color = (black_disk) ? cv::Scalar::all(0) : cv::Scalar(r, g, b, 255);
